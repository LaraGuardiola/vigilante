export class CameraClient {
  ...
  init() {
    ...
    this.cameraNameInput.value =
      localStorage.getItem("cameraName") || "My Camera";
  }
  ...
  async startStreaming() {
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert(
          "❌ Your browser does not support camera access. Use Chrome, Firefox, or Safari.",
        );
        return;
      }
      ...
      if (
        ...
        errorMsg =
          "❌ Configuration error. Make sure you are on HTTPS or localhost.";
      }
...
export class ViewerClient {
  ...
  private cameras = new Map<
    string,
    { card: HTMLElement; fpsInterval: ReturnType<typeof setInterval> }
  >();
  private ws: WebSocket | null = null;
  private reconnectTimeout: ReturnType<typeof setTimeout> | null = null;
  private enableSound = false;
  private alertSound: HTMLAudioElement;
  private soundDuration = 2000;
  ...
  constructor(options: { camerasGrid: HTMLElement; cameraUrlEl: HTMLElement }) {
    ...
    this.alertSound.addEventListener("loadedmetadata", () => {
      this.soundDuration = this.alertSound.duration * 1000 + 1000;
    });
  }

  init() {
    this.setupCameraUrl();
    this.enableSound = confirm(
      "Enable sound effects for person detection alerts?",
    );
    this.connectWebSocket();
  }
  ...
  setupCameraUrl() {
    ...
    const url =
      wsUrl.replace("wss:", "https:").replace("ws:", "http:") + "/camera";
  ...
  handleCameraList(camerasList) {
    ...
    if (camerasList.length === 0) {
      this.camerasGrid.innerHTML =
        '<div class="no-cameras">📱 No cameras connected</div>';
      return;
    }
  ...
  showPersonAlert(cameraId, count, confidence) {
    ...
    if (alertEl) {
      alertEl.textContent =
        "PERSON: " + count + " (" + confidence.toFixed(0) + "%)";
